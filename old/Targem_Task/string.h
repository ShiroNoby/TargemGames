...
namespace string
{
    ...
    private:
		...
		//
		explicit			CString(uint32_t); 	//const

		//new
		void				Allocate( uint32_t ); 	//const
	...
	public:
		...
		const char&			operator[]( uint32_t ) const; 	//const

		//operators whith range chek
		char&               at(uint32_t); 	//const
		const char&         at(uint32_t) const; 	//const
